...
class ReportHandler:
    ...
    async def process_report_request(self, message: discord.Message):
        ...
        try:
            ...
            # 检查数据库中是否有历史数据用于报告生成
            # 注意：报告生成使用数据库驱动方式，不依赖实时webhook数据
            can_generate_report = self.gemini_generator.has_sufficient_data(symbol, timeframe)
            if not can_generate_report:
                await message.reply(
                    f"❌ 未找到 {symbol} ({timeframe}) 的历史数据。\n"
                    f"请确保：\n"
                    f"1. 股票代码正确\n"
                    f"2. TradingView已推送过该时间框架的数据\n"
                    f"3. 数据库中存在相关记录"
                )
                return
    ...
    async def handle_specific_report_request(self, message: discord.Message, symbol: str, timeframe: str):
        """处理特定的RP命令报告请求"""
        try:
            self.logger.info(f"开始处理RP报告请求: {symbol} {timeframe}")
            ...
            if report:
                ...
                try:
                    embed = discord.Embed(
                        title=f"📊 {symbol} {timeframe.upper()} AI分析报告",
                        description=report[:4096] if len(report) > 4096 else report,  # Discord embed限制
                        color=0x00ff00,
                        timestamp=datetime.now(pytz.timezone('US/Eastern'))
                    )
                    
                    embed.set_footer(text="TDbot-tradingview | 仅供参考，不构成投资建议")
                    
                    await message.author.send(embed=embed)
                    
                    # 在原频道发送确认消息 - 使用原始格式
                    current_time = datetime.now(pytz.timezone('US/Eastern')).strftime("%H:%M")
                    await message.reply(f"📊 {symbol} {timeframe.upper()} AI分析报告已发送到您的私信；报告生成时间：美国东部时间•今天{current_time}")
                ...
                await message.reply("❌ 生成报告失败，请稍后重试")
                
        except Exception as e:
            self.logger.error(f"处理RP报告请求失败: {e}")
            await message.reply("❌ 处理报告请求时发生错误，请稍后重试")
