...
class TradingViewHandler:
    ...
    def parse_webhook_data(self, webhook_payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            parsed_data = {
                ...
                # 供需区信息
                'last_supply_text': body.get('lastSupplyText', ''),
                'last_demand_text': body.get('lastDemandText', ''),
    ...
    def store_enhanced_data(self, raw_payload: Dict) -> bool:
        ...
        try:
            ...
            self.logger.info(f"检测到数据类型: {data_type}")
            
            # 提取基本信息
            symbol, timeframe = self._extract_basic_info(raw_payload, data_type)
            self.logger.info(f"提取信息 - Symbol: {symbol}, Timeframe: {timeframe}")
            ...
            if data_type == 'trade':
                ...
                self.logger.info(f"提取交易字段 - Action: {tv_data.action}, Quantity: {tv_data.quantity}")
    ...
    def _extract_rating_fields(self, tv_data, data: Dict):
        ...
        # 提取供需区信息 - 新增字段
        tv_data.last_supply_text = self._safe_str(data.get('lastSupplyText'))
        tv_data.last_demand_text = self._safe_str(data.get('lastDemandText'))
        
        # 提取订单块数据 (OB数据)
        tv_data.ob_data = self._safe_str(data.get('obData'))
