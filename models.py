...
class TradingViewData(Base):
    ...
    # 供需区信息 (支持所有数据类型)
    last_supply_text = Column(String(50), nullable=True)  # 最新供应区文本，如 "163.56 -165.32 | 15MIN"
    last_demand_text = Column(String(50), nullable=True)  # 最新需求区文本，如 "155.54 - 157.84 | 15MIN"
    
    # 订单块数据 (OB数据)
    ob_data = Column(String(200), nullable=True)  # 订单块数据，如 "Timeframe: 15; Bullish OB: 344.86 - 344.04; Bearish OB: 349.54 - 348.98"
...
class ReportCache(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'last_supply_text': self.last_supply_text,
            'last_demand_text': self.last_demand_text,
            'ob_data': self.ob_data,
...
class UserWebhook(Base):
    """用户个人Webhook表 - 为每个Discord用户分配独立的TradingView Webhook URL"""
    __tablename__ = 'user_webhooks'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(String(50), nullable=False, unique=True, index=True)  # Discord用户ID
    username = Column(String(100), nullable=False)  # Discord用户名
    webhook_secret = Column(String(50), nullable=False, unique=True)  # 随机生成的密钥
    webhook_url = Column(String(500), nullable=False)  # 完整的webhook URL
    is_active = Column(Boolean, default=True, nullable=False)  # 是否激活
    message_count = Column(Integer, default=0, nullable=False)  # 收到的消息计数
    last_used = Column(DateTime, nullable=True)  # 最后使用时间
    created_at = Column(DateTime, default=func.now(), nullable=False)
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now(), nullable=False)
    
    def __repr__(self):
        return f"<UserWebhook(user_id='{self.user_id}', username='{self.username}', active={self.is_active})>"


class UserWebhookMessage(Base):
    """用户Webhook消息记录表 - 记录通过个人Webhook接收的TradingView Alert"""
    __tablename__ = 'user_webhook_messages'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(String(50), nullable=False, index=True)  # Discord用户ID
    webhook_secret = Column(String(50), nullable=False, index=True)  # 对应的webhook密钥
    symbol = Column(String(20), nullable=True, index=True)  # 股票代码（如果能解析到）
    timeframe = Column(String(10), nullable=True)  # 时间框架（如果能解析到）
    alert_message = Column(Text, nullable=False)  # TradingView Alert原始消息
    processed_message = Column(Text, nullable=True)  # 处理后的Discord格式消息
    is_sent = Column(Boolean, default=False, nullable=False)  # 是否已发送到Discord
    send_error = Column(Text, nullable=True)  # 发送错误信息（如果有）
    received_at = Column(DateTime, default=func.now(), nullable=False)
    sent_at = Column(DateTime, nullable=True)  # 发送到Discord的时间
    
    def __repr__(self):
        return f"<UserWebhookMessage(user_id='{self.user_id}', symbol='{self.symbol}', sent={self.is_sent})>"


class TradersPostConfig(Base):
    """用户TradersPost Webhook配置表"""
    __tablename__ = 'traderspost_configs'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(String(50), nullable=False, unique=True, index=True)  # Discord用户ID
    username = Column(String(100), nullable=False)  # Discord用户名
    webhook_url = Column(String(500), nullable=False)  # TradersPost Webhook URL
    ...
    def __repr__(self):
        return f"<TradersPostConfig(user_id='{self.user_id}', username='{self.username}', active={self.is_active})>"
